...
static void
fifo_merged_requests(struct request_queue *q, struct request *rq,
		    struct request *next)
{
	/*
...
static void
fifo_add_request(struct request_queue *q, struct request *rq)
{
	struct fifo_data *sd = q->elevator->elevator_data;
...
static int
fifo_queue_empty(struct request_queue *q)
{
	struct fifo_data *sd = q->elevator->elevator_data;
...
fifo_expired_request(struct fifo_data *sd, int sync, int data_dir)
{
...
static struct request *
fifo_choose_expired_request(struct fifo_data *sd)
{
	struct request *rq;
	 ...
	 */
	rq = fifo_expired_request(sd, ASYNC, WRITE);
	if (rq)
		return rq;
	rq = fifo_expired_request(sd, ASYNC, READ);
	if (rq)
		return rq;

	rq = fifo_expired_request(sd, SYNC, WRITE);
	if (rq)
		return rq;
	rq = fifo_expired_request(sd, SYNC, READ);
	if (rq)
		return rq;
...
static struct request *
fifo_choose_request(struct fifo_data *sd, int data_dir)
{
...
static inline void
fifo_dispatch_request(struct fifo_data *sd, struct request *rq)
{
	/*
...
static int
fifo_dispatch_requests(struct request_queue *q, int force)
{
	struct fifo_data *sd = q->elevator->elevator_data;
	...
	if (sd->batched > sd->fifo_batch) {
		sd->batched = 0;
		rq = fifo_choose_expired_request(sd);
	}
	...
	if (!rq) {
		...
		rq = fifo_choose_request(sd, data_dir);
		if (!rq)
			return 0;
	}

	/* Dispatch request */
	fifo_dispatch_request(sd, rq);

	return 1;
}

static struct request *
fifo_former_request(struct request_queue *q, struct request *rq)
{
	struct fifo_data *sd = q->elevator->elevator_data;
	const int sync = rq_is_sync(rq);
	const int data_dir = rq_data_dir(rq);
...
static struct request *
fifo_latter_request(struct request_queue *q, struct request *rq)
{
	struct fifo_data *sd = q->elevator->elevator_data;
	const int sync = rq_is_sync(rq);
	const int data_dir = rq_data_dir(rq);
...
static void *
fifo_init_queue(struct request_queue *q)
{
	struct fifo_data *sd;
...
static void
fifo_exit_queue(struct elevator_queue *e)
{
	struct fifo_data *sd = e->elevator_data;
...
static ssize_t
fifo_var_show(int var, char *page)
{
	return sprintf(page, "%d\n", var);
}

static ssize_t
fifo_var_store(int *var, const char *page, size_t count)
{
	char *p = (char *) page;
...
{									\
	...
	return fifo_var_show(__data, (page));			\
}
SHOW_FUNCTION(fifo_sync_read_expire_show, sd->fifo_expire[SYNC][READ], 1);
SHOW_FUNCTION(fifo_sync_write_expire_show, sd->fifo_expire[SYNC][WRITE], 1);
SHOW_FUNCTION(fifo_async_read_expire_show, sd->fifo_expire[ASYNC][READ], 1);
SHOW_FUNCTION(fifo_async_write_expire_show, sd->fifo_expire[ASYNC][WRITE], 1);
SHOW_FUNCTION(fifo_fifo_batch_show, sd->fifo_batch, 0);
SHOW_FUNCTION(fifo_writes_starved_show, sd->writes_starved, 0);
#undef SHOW_FUNCTION
...
{									\
	struct fifo_data *sd = e->elevator_data;			\
	int __data;							\
	int ret = fifo_var_store(&__data, (page), count);		\
	if (__data < (MIN))						\
		__data = (MIN);						\
	else if (__data > (MAX))					\
		__data = (MAX);						\
...
STORE_FUNCTION(fifo_sync_read_expire_store, &sd->fifo_expire[SYNC][READ], 0, INT_MAX, 1);
STORE_FUNCTION(fifo_sync_write_expire_store, &sd->fifo_expire[SYNC][WRITE], 0, INT_MAX, 1);
STORE_FUNCTION(fifo_async_read_expire_store, &sd->fifo_expire[ASYNC][READ], 0, INT_MAX, 1);
STORE_FUNCTION(fifo_async_write_expire_store, &sd->fifo_expire[ASYNC][WRITE], 0, INT_MAX, 1);
STORE_FUNCTION(fifo_fifo_batch_store, &sd->fifo_batch, 0, INT_MAX, 0);
STORE_FUNCTION(fifo_writes_starved_store, &sd->writes_starved, 0, INT_MAX, 0);
#undef STORE_FUNCTION

#define DD_ATTR(name) \
	__ATTR(name, S_IRUGO|S_IWUSR, fifo_##name##_show, \
				      fifo_##name##_store)

static struct elv_fs_entry fifo_attrs[] = {
...
static struct elevator_type iosched_fifo = {
	.ops = {
		.elevator_merge_req_fn		= fifo_merged_requests,
		.elevator_dispatch_fn		= fifo_dispatch_requests,
		.elevator_add_req_fn		= fifo_add_request,
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,38)
		.elevator_queue_empty_fn	= fifo_queue_empty,
#endif
		.elevator_former_req_fn		= fifo_former_request,
		.elevator_latter_req_fn		= fifo_latter_request,
		.elevator_init_fn		= fifo_init_queue,
		.elevator_exit_fn		= fifo_exit_queue,
	},

	.elevator_attrs = fifo_attrs,
	.elevator_name = "fifo",
